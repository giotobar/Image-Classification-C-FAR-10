...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    #soft_max = np.exp(scores)/np.sum(np.exp(scores),axis=1)
    #corr_prob = -np.log(soft_max[np.arange(N),y])
    
    ai = scores[np.arange(N),y]
    softmax_prob = np.exp(ai)/np.sum(np.exp(scores),axis=1)
    corr_loss = -np.sum(np.log(softmax_prob))
    corr_loss /= N
    
    W1_penalty = 0.5*reg*np.sum(W1*W1) 
    W2_penalty = 0.5*reg*np.sum(W2*W2)
    
    loss = corr_loss + W1_penalty + W2_penalty
	...
	# ================================================================ #
    total_softmax = np.exp(scores)/np.sum(np.exp(scores),axis=1,keepdims=True)
    total_softmax[np.arange(N),y] -= 1
    total_softmax /= N
    h2_back = np.dot(total_softmax,W2)
    f1_back = h2_back*(i1>0)
    
    
    grads['W2'] = np.dot(total_softmax.T,h1)
    grads['b2'] = np.sum(total_softmax.T,axis=1)
    grads['W1'] = np.dot(f1_back.T,X)
    
   
    grads['b1'] = np.sum(f1_back.T,axis=1)
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in np.arange(num_iters):
      ...
      # YOUR CODE HERE:
  	  ...
  	  # ================================================================ #
        grab_batch = np.random.choice(num_train,batch_size)
        X_batch = X[grab_batch]
        y_batch = y[grab_batch]
  ...
  def predict(self, X):
    ...
    # ================================================================ #
    h1 = np.matmul(X,self.params['W1'].T)+ self.params['b1']
    
    f1 = np.maximum(0,h1)
    
    h2 = np.matmul(f1,self.params['W2'].T)+ self.params['b2']
