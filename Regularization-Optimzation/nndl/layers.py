...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
        cache = (sample_mean,sample_var,norm_x,gamma,beta,bn_param,x)
        # ================================================================ #
        # END YOUR CODE HERE
        # ================================================================ #
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    #cache = (sample_mean,sample_var,running_mean,running_var,norm_x,gamma,beta,bn_param,x)
    # ================================================================ #
    # YOUR CODE HERE:
    #   Implement the batchnorm backward pass, calculating dx, dgamma, and dbeta.
    # ================================================================ #
    sample_mean,sample_var,norm_x,gamma,beta,bn_param,x = cache
    eps = bn_param.get('eps', 1e-5)
    N,D = x.shape
    ...
    da = (1 / np.sqrt(sample_var+eps))*dnorm_x
    
    de = (-0.5)* (1/np.power(sample_var+eps,1.5))*(x-sample_mean)*dnorm_x
    
    dvar = np.sum(de,axis =0)
   
    du1 = -da
    du2 = dvar*(2/N)*np.sum(x-sample_mean,axis =0)
    
    du = du1+du2
    
    du2 = -dvar* np.ones((N, D))*(2/N)*(x-sample_mean)
    
    du = np.sum(-da +du2,axis =0)
    
    dx1 = ((2*(x-sample_mean))/N)*dvar
